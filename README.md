...
**[最新リリース](../../releases/latest)** から入手できます (Apple Silicon / Intel 両対応)。

| ファイル | 内容 |
|---|---|
| `AtermDetector-v*.dmg` | GUI アプリ (開いて Applications へドラッグ) |
| `aterm-cli-v*-macos-universal.tar.gz` | コマンドライン版 `aterm-cli` |

### インストール (GUI アプリ)

1. `AtermDetector-v*.dmg` をダブルクリックして開く
2. 表示されたウィンドウで **AtermDetector を Applications フォルダにドラッグ**
3. アプリケーションフォルダから起動
4. 初回起動時の「ローカルネットワーク」許可ダイアログで **「許可」** をクリック

> **重要**: 必ず Applications フォルダに入れてから起動してください。ダウンロードフォルダから直接起動すると、macOS の保護機能 (App Translocation) によりローカルネットワーク権限が付与されず、Aterm を検出できません。
...
![aterm-cli の実行例](docs/images/cli.png)
