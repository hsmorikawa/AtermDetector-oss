...
GUI を使わずターミナルから検出したい場合や、結果をスクリプトで処理したい場合に使えます。

```bash
# ビルド
xcodegen generate
xcodebuild build -scheme aterm-cli -configuration Release -derivedDataPath build
CLI=build/Build/Products/Release/aterm-cli

# サブネット全体をスキャンして一覧表示
$CLI

# clean モード: タブ区切り・ヘッダなし (パイプ / スクリプト向け)
$CLI --clean            # IP<TAB>機種名<TAB>動作モード<TAB>MAC<TAB>URL
